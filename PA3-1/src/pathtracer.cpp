...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
  ...
  for(int i = 0; i < num_samples; ++i){
    ...
    Ray shadow = Ray(EPS_D * wi_world + hit_p, wi_world);
  ...
  if(b || r.depth < max_ray_depth){
    ...
    if(bvh->intersect(shadow, &intersection)){
      if(r.depth < max_ray_depth){
        L_out += at_least_one_bounce_radiance(shadow, intersection) * sp * cos_theta(w_in) / pdf;
      }
      else{
        L_out += at_least_one_bounce_radiance(shadow, intersection) * sp * cos_theta(w_in) / pdf / p;
      }
    }
  }
  return L_out;
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  double u, sigma, sigma2, I;
  for (int i = 0; i < num_samples; ++i){
  	if(num_samples == 1){
  		r = camera->generate_ray(((double)x+0.5)/(double)sampleBuffer.w, ((double)y+0.5)/(double)sampleBuffer.h);
  	}
  	else{
  		Vector2D v = gridSampler->get_sample();
  		v += origin;
  		r = camera->generate_ray((double)v.x/(double)sampleBuffer.w, (double)v.y/(double)sampleBuffer.h);
  	}
    sp = est_radiance_global_illumination(r);
    average += sp;
    s1 += sp.illum();
    s2 += sp.illum() * sp.illum();
    if( (i+1) % samplesPerBatch == 0){
      u = s1 / (double)(i+1);
      sigma2 = 1.0 / i * (s2 - (s1 * s1) / (double)(i+1));
      sigma = sqrt(sigma2);
      I = 1.96 * sigma / sqrt(i+1);
      if(I <= maxTolerance * u){

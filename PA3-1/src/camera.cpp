...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // 
  Vector3D bottom_left(-tan(radians(hFov)*.5), -tan(radians(vFov)*.5),-1);
  Vector3D top_right(tan(radians(hFov)*.5),  tan(radians(vFov)*.5),-1);
  double X = bottom_left.x + (top_right.x - bottom_left.x) * x;
  double Y = bottom_left.y + (top_right.y - bottom_left.y) * y;
  double Z   = -1.0;
  Vector3D origin = pos;
  Vector3D direction = c2w * Vector3D(X,Y,Z);
  direction.normalize();
  return Ray(origin,direction,fClip);
}

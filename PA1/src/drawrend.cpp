...
float line_function(float x, float y, float x1, float y1, float x2, float y2){
  return -(x-x1) * (y2-y1) + (y-y1) * (x2-x1);
}

bool inTri(float x, float y, float x0, float y0, float x1, float y1, float x2, float y2){
  bool b0 = line_function(x,y,x0,y0,x1,y1) >= 0;
  bool b1 = line_function(x,y,x1,y1,x2,y2) >= 0;
  bool b2 = line_function(x,y,x2,y2,x0,y0) >= 0;
  return ((b0 == b1) && (b1 == b2));
}

Vector3D barycentric(float x, float y, float x0, float y0, float x1, float y1, float x2, float y2){
  float alpha = line_function(x,y,x1,y1,x2,y2) / line_function(x0,y0,x1,y1,x2,y2);
  float beta = line_function(x,y,x2,y2,x0,y0) / line_function(x1,y1,x2,y2,x0,y0);
...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  int samples_per_side = sqrt(sample_rate);
    float sub_pixel_length = 1.0/samples_per_side;
  float x = 0; float y = 0;
    ...
    for (int j = min_x; j < max_x; ++j){
      for(int k = 0; k < samples_per_side; ++k){
        for(int t = 0; t < samples_per_side; ++t){
          x = j + k * sub_pixel_length + sub_pixel_length / 2.0;          
          y = i + t * sub_pixel_length + sub_pixel_length / 2.0;
            if(inTri(x,y,x0,y0,x1,y1,x2,y2)){
              if(tri == NULL)
                ...
                sp.lsm = lsm;
                sp.psm = psm;
                Vector3D bary_coor = barycentric(x,y,x0,y0,x1,y1,x2,y2);
                Vector3D dx_bary = barycentric(x+1,y,x0,y0,x1,y1,x2,y2);
                Vector3D dy_bary = barycentric(x,y+1,x0,y0,x1,y1,x2,y2);
                samplebuffer[i][j].fill_color(k,t,tri->color(bary_coor, dx_bary, dy_bary, sp));
              }
            }
        }

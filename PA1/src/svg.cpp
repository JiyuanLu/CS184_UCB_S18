...
Color ColorTri::color(Vector3D p_bary, Vector3D p_dx_bary, Vector3D p_dy_bary, SampleParams sp) {
  // Part 4: Fill this in.
  return p_bary.x * this->p0_col + p_bary.y * this->p1_col + p_bary.z * this->p2_col;
}
...
Color TexTri::color(Vector3D p_bary, Vector3D p_dx_bary, Vector3D p_dy_bary, SampleParams sp) {
  ...
  Vector2D uv = p_bary.x * this -> p0_uv + p_bary.y * this -> p1_uv + p_bary.z * this -> p2_uv;
  sp.p_uv = uv;
  Vector2D dx_uv = p_dx_bary.x * this -> p0_uv + p_dx_bary.y * this -> p1_uv + p_dx_bary.z * this -> p2_uv;
  sp.p_dx_uv = dx_uv;
  Vector2D dy_uv = p_dy_bary.x * this -> p0_uv + p_dy_bary.y * this -> p1_uv + p_dy_bary.z * this -> p2_uv;
  sp.p_dy_uv = dy_uv;
